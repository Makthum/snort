...
typedef struct _ARPReqListNode
{
ARPRequestQ *arpQ;
struct _ARPReqList *next;
}ARPReqlistNode;

typedef struct _ARPReqList
{
int size;
ARPReqlistNode *head;
ARPReqlistNode *tail;
...
static void AddtoQueue(uint32_t ipv4_addr);
...
static void AddtoQueue(uint32_t ipv4)
{
if(reqlist==NULL)
{
reqlist=(ARPRequestList *)SnortAlloc(sizeof(ARPRequestList));
ARPReqlistNode *temp=(ARPReqlistNode *)SnortAlloc(sizeof(ARPReqlistNode));
temp->arpQ->ipv4_addr=ipv4;
temp->arpQ->arp_status=1;
reqlist->head=temp;
reqlist->tail=temp;
}
else
...
static void PrintQueue()
{
ARPReqlistNode *temp=reqlist->head;
while(temp!=NULL)
{
printf("%s",temp->arpQ->ipv4_addr);
temp=temp->next;
}
}
...
static void ARPspoofInit(char *args)
{
    ...
    LogMessage("Preprocessor: ARPspoof Initialized\n");
...
static void ARPspoofHostInit(char *args)
{
    ...
    LogMessage("Preprocessor: ARPspoof (overwrite list) Initialized\n");
...
static void DetectARPattacks(Packet *p, void *context)
{
    ...
    switch(ntohs(p->ah->ea_hdr.ar_op))
    {
        case ARPOP_REQUEST:
                    ...
                    (u_char *)p->ah->arp_sha, 6) != 0)
            {
                ...
                LogMessage("MODNAME: Ethernet/ARP mismatch request\n");
            }
            break;
        case ARPOP_REPLY:
                    ...
                    (u_char *)p->ah->arp_sha, 6) != 0)
            {
                ...
                LogMessage("MODNAME: Ethernet/ARP mismatch reply src\n");
            }
                    ...
                    (u_char *)p->ah->arp_tha, 6) != 0)
            {
                ...
                LogMessage("MODNAME: Ethernet/ARP mismatch reply dst\n");
            }
            break;
    }
    ...
    {
        LogMessage("MODNAME: LookupIPMacEntryByIp returned NULL\n");
        return;
    }
    else
    {
        LogMessage("MODNAME: LookupIPMacEntryByIP returned %p\n", ipme);
                ...
                (uint8_t *)ipme->mac_addr, 6)))
        {
            ...
            LogMessage("MODNAME: Attempted ARP cache overwrite attack\n");
...
# endif
        ...
        LogMessage("MODNAME: LookupIPMacEntryByIP() comparing %s to %s\n", cha, chb);
#endif
        if (current->ip_mac_entry->ipv4_addr == ipv4_addr)
        {
            LogMessage("MODNAME: LookupIPMecEntryByIP() match!");
